...
test.describe.serial('家庭运营与回收站', () => {
  test('管理员可查看运营记录、恢复并永久删除物品', async ({ page }) => {
    ...
    try {
      ...
      const deletedActivity = page.locator('.activity-entry').filter({ hasText: itemName })
      await expect(deletedActivity).toHaveCount(1)
      await expect(deletedActivity).toBeVisible()
